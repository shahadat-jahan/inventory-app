...
function goCreate() { router.get('/customers/create') }
function editItem(id: number) { router.get(`/customers/${id}/edit`) }
function destroyItem(id: number) { if (confirm('Delete customer?')) router.delete(`/customers/${id}`) }
</script>
          ...
          <tbody>
            <tr v-for="c in props.customers.data" :key="c.id" class="border-b">
              ...
              <td class="py-2 pr-4 space-x-2">
                ...
                <Button size="sm" variant="destructive" @click="destroyItem(c.id)">Delete</Button>
              </td>
            </tr>
          </tbody>
        </table>
