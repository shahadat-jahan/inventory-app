...
function confirmDelete() {
    if (itemToDelete.value) {
        router.delete(`/warehouses/${itemToDelete.value.id}`, {
            onSuccess: () => {
                if (window.showToast) {
                    window.showToast('success', 'Warehouse deleted successfully');
                }
            },
            onError: () => {
                if (window.showToast) {
                    window.showToast('error', 'Failed to delete warehouse');
                }
            },
        });
    }
...
<template>
    <AppLayout>
                    ...
                    <tbody>
                        <tr v-for="w in props.warehouses.data" :key="w.id" class="border-b">
                            ...
                            <td class="space-x-2 py-2 pr-4">
                                ...
                                <Button size="sm" variant="destructive" @click="showDeleteConfirm(w)">Delete</Button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <DeleteConfirmModal
            ...
            :message="`Are you sure you want to delete the Warehouse '${itemToDelete?.name || ''}'?  This action cannot be undone.`"
