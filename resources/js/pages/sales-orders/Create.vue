...
function submit() {
  router.post('/sales-orders', form, {
    onSuccess: () => {
      if (window.showToast) {
        window.showToast('success', 'Sales order created successfully')
      }
    },
    onError: () => {
      if (window.showToast) {
        window.showToast('error', 'Failed to create sales order')
      }
    }
  })
}
...
<template>
  <AppLayout>
    <div class="p-6 space-y-6">
      <Heading title="New S ales Order" />
